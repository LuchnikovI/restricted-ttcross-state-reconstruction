...
def main(argv):
    ...
    controlled_id_im_prediction = np.array([[float(v) for v in bloch_vec] for bloch_vec in data["controlled_id_im_prediction"][-1]])
    config = data["config"]
    ...
    plt.legend(['Exact dynamics', 'Predicted_dynamics'])
    plt.ylabel("<X>")
    ...
    plt.plot(controlled_id_im_prediction[:, 0], "-xk")
    plt.legend(['Exact dynamics', 'Predicted_dynamics', 'ID IM dynamics'])
    plt.ylabel("<X>")
    plt.ylim(top=1, bottom=-1)
    plt.xlabel("Time")
